...
function isMultipleOfBothThreeAndFive(number) {
	var threeremainder = (number % 3);
	var fiveremainder = (number % 5);
...
function sumOfSquares(x,y) {
	var xsquare = (x * x);
	var ysquare = (y * y);
	var sumofsquares = (xsquare + ysquare);
	return sumofsquares
}
...
function timesTwoPlusThree(number) {
	var multiply = (number * 2);
	var addingthree = (multiply + 3);
	return addingthree
}
...
function areaOfCircle(number) {
	var squaredinput = (number * number);
	return (squaredinput * Math.PI)
}
...
function endsWithVowel(input) {
	var endingchar = input.charAt(input.length-1);

	if (endingchar.match('a|e|i|o|u')) {
		return true
	} else {
		return false
	}
...
function startsAndEndsWithVowel(input) {
	return (/^[aeiou].*[aeiou]$/i).test(input)
}
...
function secondToLast (input){
	let seclast=input[input.length-2];
	return seclast
}
...
function thirdToLast (input) {
	let thirdlast=input[input.length-3];
	return thirdlast
}
...
function firstToLast (input) {
	let restructure = input;
	restructure.push(restructure.shift());
...
function productOfAll (input) {
	let product = input.reduce(function (a,b){
		return a * b
	}, 1)
	return product
}
...
function onlyEvenNumbers (input) {
	...
	let onlyeven = input.filter(even);
	return onlyeven
}
...
function onlyPositiveNumbers (input) {
	...
	let positivelist = input.filter(filter);
	return positivelist
}
...
function onlyNegativeNumbers (input) {
	...
	let negativelist = input.filter(filter);
	return negativelist
}
...
function countEvens (input) {
	let oddcut = input.reduce (function (a,b) {
		return a + b % 2;
	}, 0)

	let even = input.length - oddcut
...
function countOdds (input) {
	let odd = input.reduce (function(a,b){
		return a + b % 2;
	}, 0)
...
function onlyPositiveOdds (input) {
	...
	let positiveodds = positivelist.filter(function(number){
		return number % 2 != 0});
...
function shortestString (input) {
	return input.reduce(function(prevWord, currWord) {
...
function elementsTimesTwo (input) {
	let result = input.map(function(numbers) {
		return numbers * 2
	});
...
function highestPriceBook(input){
	...
	for (i = 0; i < books.length; i++) {
		if (books[i].price > testagainst.price) {
			testagainst.price = books[i].price;
		} else return books.find(books => books.price === testagainst.price);
	}
