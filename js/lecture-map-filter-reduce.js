...
let addedOne = numbers.map(function(n) {
    return n + 1;
})
...
let multiplyThree  = addedOne.map(num => num * 3);
...
let ones = binary.filter(n => n == true);
...
let zeroes = binary.filter(n => n == false);
console.log(zeroes);
...
let fruits = fruitsAndVeggies.filter (obj => obj.type === "fruit");
console.log(fruits);
// TO DO: filter through the array of objects and return all vegetable type
// objects.

let veggies = fruitsAndVeggies.filter (obj => obj.type === "vegetable");
console.log(veggies);
...
//     const exampleReduce = numbers.reduce((subtraction, num) => {
...
let colortest = colors.reduce((colorcounter, color) => {
    if (typeof colorcounter[color] === "undefined") {
